...
const TrussInfo = (props: TrussInfoProps) => {
	const [undo, setUndo] = usePersistentState<TrussJSONType[]>('truss_undo', [])
	...
	useEventEffect((e: KeyboardEvent) => {
		...
		switch (e.key) {
			...
			case 'z':
				if (e.ctrlKey) {
					if (undo.length) {
						const truss = Truss.fromJSON(undo.pop() as TrussJSONType)
						setUndo([ ...undo ])
	...
	// useEventEffect((e: MouseEvent) => {
		
	// 	switch (e.buttons) {
	// 		case 1:
	// 			if (!e.ctrlKey) break
	// 			console.log('ctrl+click', e.clientX, e.clientY)
	// 		break
	// 	}
					...
					<>
						...
						>
							...
							<TextField
								...
								size={'small'}
								variant={'outlined'}
								onChange={(e) => setMultiplier(Math.max(Math.floor(Number(e.target.value)), 1))}
							/>
						</Box>
					</>
				)}
